...
public class GoGuiImpl extends Application {
	...
	private void initBoardLines() {
		...
		for (int i = 1; i <= currentBoardSize; i++) {
			boardLines.add(new Line(squareSize, i * squareSize, 
					currentBoardSize * squareSize, i * squareSize));
		}

		// Draw vertical lines
		for (int i = 1; i <= currentBoardSize; i++) {
			boardLines.add(new Line(i * squareSize, squareSize, 
					i * squareSize, currentBoardSize * squareSize));
		}
	...
	protected void addAreaIndicator(int x, int y, boolean white) throws InvalidCoordinateException {
		...
		if (mode3D) {
			Box areaStone = new Box(currentSquareSize / 3, 
					currentSquareSize / 3, currentSquareSize / 3);
